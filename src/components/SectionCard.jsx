...
export default function SectionCard({ item, section, index }) {
  return (
    <Link to={`/detail/${section}/${index}`}>
      <article className="flex flex-col justify-between h-full bg-gradient-to-tr from-black/60 to-navy-900/60 border border-gray-800 rounded-xl p-5 shadow-md hover:scale-[1.05] transition-transform cursor-pointer">
        <div>
...
//   return (
//     <Link to={`/detail/${section}/${index}`}>
//       <article className="bg-gradient-to-tr from-black/60 to-navy-900/60 border border-gray-800 rounded-xl p-5 shadow-md hover:scale-[1.05] transition-transform cursor-pointer">
//         <div className="flex items-start justify-between gap-4">
//           <div>
//             <h3 className="font-semibold text-gray-100">{item.title}</h3>
//             {item.subtitle && <p className="text-sm text-gray-300 mt-1">{item.subtitle}</p>}
